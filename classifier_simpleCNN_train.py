...
def main():
    ...
    # 4.2 Data Transforms
    # ===========================
    # ImageNet statistics for normalization (optional for simple CNN)
    ...
    data_transforms = {
        ...
        'test': transforms.Compose([
            transforms.Resize((256, 256)),  # Changed to 256 for consistency
            transforms.CenterCrop(224),     # Added CenterCrop for consistency
            transforms.ToTensor(),
            transforms.Normalize(mean=imagenet_mean, std=imagenet_std)
        ]),
    }
    ...
    model_path = os.path.join(save_dir, 'simple_cnn_model.pth')  # Changed filename to reflect the new model
    ...
    for i in range(min(6, inputs.size(0))):
        ...
        inp = inp * torch.tensor(imagenet_std).view(3,1,1) + torch.tensor(imagenet_mean).view(3,1,1)  # Unnormalize
